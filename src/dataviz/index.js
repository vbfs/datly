...
class DataViz {
  ...
  createContainer(width = this.defaultWidth, height = this.defaultHeight) {
    let container = d3.select(`#${this.containerId}`);
  ...
  boxplot(data, options = {}) {
    ...
    // Process data - can be array of arrays or single array
  ...
  scatter(xData, yData, options = {}) {
    ...
    const data = xData.map((x, i) => ({ x, y: yData[i], label: labels ? labels[i] : null }));
    ...
    g.selectAll('circle')
      ...
      .on('mouseover', function(event, d) {
        d3.select(this).attr('r', size * 1.5).attr('opacity', 1);
        tooltip.style('opacity', 1)
          .html(`X: ${d.x.toFixed(2)}<br>Y: ${d.y.toFixed(2)}${d.label ? '<br>' + d.label : ''}`)
  ...
  heatmap(matrix, options = {}) {
    ...
    const rowLabels = labels || Array.from({ length: n }, (_, i) => `Var${i + 1}`);
    const colLabels = labels || Array.from({ length: n }, (_, i) => `Var${i + 1}`);
    ...
    const kde = (data, bandwidth = 0.5) => {
      ...
      return thresholds.map(t => {
        const density = d3.mean(data, d => {
          return Math.exp(-0.5 * Math.pow((d - t) / bandwidth, 2)) / (bandwidth * Math.sqrt(2 * Math.PI));
        });
        return [t, density];
      });
    };
  ...
  density(data, options = {}) {
    ...
    const bw = bandwidth || 1.06 * d3.deviation(data) * Math.pow(data.length, -1/5);

    // Generate density points
    const extent = d3.extent(data);
    const range = extent[1] - extent[0];
    const xPoints = d3.range(extent[0] - range * 0.1, extent[1] + range * 0.1, range / 200);

    const densityData = xPoints.map(xi => {
      const density = d3.mean(data, d => {
        return Math.exp(-0.5 * Math.pow((d - xi) / bw, 2)) / (bw * Math.sqrt(2 * Math.PI));
      });
